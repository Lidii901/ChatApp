...
import { Character, ChatLanguage, ChatSession, ChatCharacterSettings } from '../types';
      ...
      <div className="flex max-h-[92vh] w-full flex-col overflow-hidden rounded-t-[30px] border border-zinc-800 bg-[#0b0b0d] shadow-2xl sm:max-w-2xl sm:rounded-[28px]" onClick={event => event.stopPropagation()}>
        <header className="flex items-center justify-between border-b border-zinc-900 px-5 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-rose-400">Nur dieser Chat</p>
              <h2 className="text-base font-black text-white">Chat & Character Settings</h2>
            </div>
          </div>
        ...
        <div className="flex-1 space-y-5 overflow-y-auto px-5 py-5 text-sm text-zinc-300">
          <div className="grid gap-4 sm:grid-cols-2">
            <Field label="Chatname"><input className="input" value={title} onChange={e => setTitle(e.target.value)} /></Field>
            <Field label="Sprache">
              <div className="grid grid-cols-2 gap-1 rounded-2xl bg-zinc-900 p-1.5">
                {(['de', 'en'] as const).map(value => (
                  <button key={value} onClick={() => setLanguage(value)} className={`flex items-center justify-center gap-1.5 rounded-xl py-2.5 text-xs font-black ${language === value ? 'bg-rose-600 text-white' : 'text-zinc-500'}`}>
                    <Globe2 className="h-3.5 w-3.5" /> {value.toUpperCase()}
                  </button>
                ))}
              </div>
            </Field>
          ...
          <div className="rounded-2xl border border-zinc-800 bg-zinc-950/70 p-4">
            ...
            <p className="mt-1 text-[11px] leading-relaxed text-zinc-600">Die Zusammenfassung wird als zusätzlicher Gesprächskontext an das Modell gesendet. Das entspricht dem Grundprinzip von Chub Chat Memory.</p>
            <div className="mt-4 space-y-4">
              ...
              <Field label="Chat-Memory-Zusammenfassung"><textarea className="input" rows={5} value={sceneSummary} onChange={e => setSceneSummary(e.target.value)} placeholder="Wichtige Entwicklungen des bisherigen Chats …" /></Field>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-rose-900/25 bg-rose-950/10 p-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-xs font-black text-rose-200">Character Definition für diesen Chat</p>
                <p className="mt-1 text-[11px] leading-relaxed text-zinc-500">Diese Overrides werden direkt auf die V2-Promptfelder gelegt. Keine Dominanz-/Humor-/Pacing-Regler, die der Prompt danach ignoriert.</p>
              </div>
        ...
        <footer className="flex justify-end border-t border-zinc-900 px-5 py-4">
          <button onClick={handleSave} className="flex items-center gap-2 rounded-2xl bg-rose-600 px-5 py-3 text-sm font-black text-white">
            <Save className="h-4 w-4" /> {saved ? 'Gespeichert' : 'Übernehmen'}
          </button>
        </footer>
